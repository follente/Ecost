...
import { ConversionFactor } from '../entities/conversionFactor.entity';

@Injectable()
export class ConversionFactorService {

  constructor(
    @InjectModel( ConversionFactor.name )
    private conversionFactorModel: Model<ConversionFactor>,
  ){}


  async create(createResultDto: CreateResultDto): Promise<ConversionFactor> {
    try {
      const newResult = new this.conversionFactorModel( createResultDto );
  ...
  findAll() {
    return this.conversionFactorModel.find();
  }
