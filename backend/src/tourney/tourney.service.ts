...
export class TourneyService {
  ...
  constructor(
    @InjectModel( Tourney.name )
    private tourneyModel: Model<Tourney>,
  ){}


  async create(createTourneyDto: CreateTourneyDto): Promise<Tourney> {

    try {
      const newTourney = new this.tourneyModel( createTourneyDto );
      await newTourney.save();
      return newTourney.toJSON();
      
    } catch (error) {
      if(error.code === 11000){
        throw new BadRequestException(`${ createTourneyDto.name } already exists!`)
      }
